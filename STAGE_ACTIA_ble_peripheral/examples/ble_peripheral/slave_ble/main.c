...
#define STATIC_PIN "000000" //  code PIN fixe
#define DEVICE_NAME                     "ACTIA ES"                  // Nom de l'appareil diffusé dans les données publicitaires.
...
#define SEC_PARAM_MITM                  1                                       //  Protection contre les attaques Man-In-The-Middle (MITM) requise.
...
static void gap_params_init(void)
{
    ...
    // Définir le mode de sécurité sur MITM avec code PIN
    ...
    // Configurer le code PIN fixe
    memset(&opt, 0, sizeof(opt));
    opt.gap_opt.passkey.p_passkey = (uint8_t *)STATIC_PIN;
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        {
            NRF_LOG_INFO("BLE_GAP_EVT_AUTH_KEY_REQUEST: Sending PIN");
            // Réponse avec le code PIN fixe
...
int main(void)
{
    ...
    conn_params_init();
    peer_manager_init();
    NRF_LOG_INFO("peripheral started.");
